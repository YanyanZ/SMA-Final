...
#include "../../generator/include/noise/noise.h"
#include "../../generator/src/noiseutils.h"
...
int Game::create_planet(int i)
{
  ...
  std::stringstream ss;
  ss << "maps/" << i << ".bmp";
  bitmapWriter.SetDestFilename (ss.str());
  ...
  bitmapWriter.WriteDestFile (tmp);
    ...
    for (int i = 0; i < 800; ++i)
    {
      ...
      for (int j = 0; j < 800; ++j)
        tmp_i.push_back(std::make_pair<uchar, Player*>(tmp[i][j], nullptr));
